...
class NormalImageProcessor : ImageProcessor {
    override val name = "NormalEffect"
    ...
    override fun blur(radius: Float, outputIndex: Int): Bitmap {
        params?.let {
            it.bitmapOut = it.bitmap.copy(Bitmap.Config.ARGB_8888, false)
            it.bitmapOut = it.bitmapOut.scale(it.bitmap.width / 4, it.bitmap.height / 4)
            it.bitmapOut = fastBlur(it.bitmapOut, radius.toInt())
            return it.bitmapOut
        }
